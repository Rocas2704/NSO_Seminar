    ...
    def solve(self, A: np.ndarray) -> np.ndarray:
        ...
        for l in range(2, self.k + 1):
            ...
            for y0 in A5:
                ...
                x0 = x_init.flatten()
                if self.method == "lmbm":
                    from LMBMSolver import LMBMSolver
                    ...
                    from HSMSolver import SmoothingSolver
                    from HSMSolver import SmoothedClusterObjective  # (k x n)
    ...
    def _generate_starting_points(self, A: np.ndarray, centers: list[np.ndarray]) -> list[np.ndarray]:
        ...
        for c in A3:
            if self.method == "lmbm":
                ...
                from HSMSolver import SmoothingSolver
                from HSMSolver import SmoothedAuxiliaryObjective
                solver = SmoothingSolver(self.config)
        ...
        print("Candidatos finales A5:", len(A5))
