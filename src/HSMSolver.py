...
class SmoothingSolver:
    ...
    def solve_objective(self, objective_fn, x0: np.ndarray) -> np.ndarray:
        ...
        for it in range(self.max_iter):
            ...
            f0 = objective_fn(x)
            grad_norm = np.linalg.norm(x - x_prev) / max(1e-8, np.abs(f0))


            elapsed = time() - start_time
            print(f"[HSM] Iter {it + 1:02d}/{self.max_iter} | f(x) = {f0:.6f} | ∥∇f∥ ≈ {grad_norm:.2e} | "
                  f"tau = {tau:.4f} | eps = {epsilon:.2e} | time = {elapsed:.2f}s")
            if grad_norm < epsilon:
                print(f"[HSM] Converged at iter {it + 1} (∥∇f∥ < ε). Total time: {elapsed:.2f}s\n")
